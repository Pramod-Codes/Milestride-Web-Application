...
export default function ReportsDashboard() {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-[var(--ms-border)] bg-[var(--ms-panel)] p-4">
      ...
      <div className="flex flex-wrap items-center gap-2"><ReportSelect label="Report" value={reportType} onChange={setReportType} options={reportTypes} /><ReportSelect label="Range" value={range} onChange={setRange} options={ranges} /><button className="ms-primary-button"><Download className="h-3.5 w-3.5" /> Export report</button></div>
    </div>
...
function ReportSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[] }) {
