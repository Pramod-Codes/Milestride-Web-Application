...
export default function MilestrideShell({ children }: { children: ReactNode }) {
  ...
  return <ThemeContext.Provider value={value}><FleetContext.Provider value={fleetValue}><div className={`ms-shell ${theme === "light" ? "ms-light" : ""}`}>
    <header className="ms-header">
      <div className="flex items-center gap-2"><button className="ms-icon-button lg:hidden" onClick={() => setMobileNavOpen(true)} aria-label="Open navigation"><Menu className="h-4 w-4" /></button><Link to="/overview" className="flex items-center gap-3"><span className="ms-logo"><Bike className="h-4 w-4" /></span><span className="text-sm font-semibold tracking-[-0.02em]">Milestride</span></Link><span className="hidden h-4 w-px bg-[var(--ms-border)] sm:block" /><span className="hidden text-xs text-[var(--ms-muted)] sm:block">Operations workspace</span><div className="relative hidden sm:block"><select aria-label="Select operating fleet" value={fleetId} onChange={(event) => setFleetId(event.target.value as FleetId)} className="h-8 max-w-[170px] appearance-none rounded-md border border-[var(--ms-border)] bg-[var(--ms-panel)] px-2 pr-7 text-[10px] font-semibold text-[var(--ms-body)] outline-none">{fleetOptions.map((fleet) => <option key={fleet.id} value={fleet.id}>{fleet.name}</option>)}</select><ChevronDown aria-hidden="true" className="pointer-events-none absolute right-2 top-1/2 h-3 w-3 -translate-y-1/2 text-[var(--ms-muted)]" /></div></div>
