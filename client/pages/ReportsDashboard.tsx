...
function ReportTable({ title, description, headers, rows }: ReportTableProps) {
  return <section className="rounded-xl border border-[var(--ms-border)] bg-[var(--ms-panel)] p-4 sm:p-5"><div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><h2 className="text-sm font-semibold">{title}</h2><p className="mt-1 text-[11px] text-[var(--ms-muted)]">{description}</p></div><button className="ms-control"><Download className="h-3.5 w-3.5" /> Download CSV</button></div><div className="max-h-[min(62vh,38rem)] overflow-auto rounded-lg border border-[var(--ms-border)]"><table className="w-full min-w-[760px] border-collapse text-left text-xs"><thead className="sticky top-0 z-10 bg-[var(--ms-panel)]"><tr className="border-b border-[var(--ms-border)] text-[10px] uppercase tracking-[0.1em] text-[var(--ms-muted)]">{headers.map((header) => <th key={header} className="border-b border-[var(--ms-border)] px-3 py-2.5 font-medium">{header}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={index} className="border-b border-[var(--ms-border)] last:border-0"><>{row.map((cell, cellIndex) => <td key={cellIndex} className="px-3 py-2.5 text-[var(--ms-muted)]">{cell}</td>)}</></tr>)}</tbody></table></div><p className="mt-3 text-[10px] text-[var(--ms-muted)]">{rows.length} records · {title} · Updated just now</p></section>;
}

function ReportMetric({ icon, label, value, tone }: { icon: ReactNode; label: string; value: string; tone: string }) { return <div className="rounded-xl border border-[var(--ms-border)] bg-[var(--ms-panel)] px-3 py-2.5"><span className={tone}>{icon}</span><p className={`mt-2 text-xl font-semibold ${tone}`}>{value}</p><p className="mt-0.5 text-[10px] text-[var(--ms-muted)]">{label}</p></div>; }
