  ...
  return <ThemeContext.Provider value={value}><FleetContext.Provider value={fleetValue}><div className={`ms-shell ${theme === "light" ? "ms-light" : ""}`}>
    ...
    <div className="flex min-h-[calc(100vh-4rem)]">
      ...
      <aside className={`ms-sidebar ${sidebarCollapsed ? "collapsed" : ""} ${mobileNavOpen ? "mobile-open" : ""}`}>
        ...
        <div className={`mt-8 border-t border-[var(--ms-border)] pt-5 ${sidebarCollapsed ? "hidden" : ""}`}><p className="px-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--ms-muted)]">System status</p><div className="mt-4 flex items-center gap-2 px-2 text-xs text-[var(--ms-body)]"><span className="h-2 w-2 flex-shrink-0 rounded-full bg-[var(--ms-accent-strong)] shadow-[0_0_10px_var(--ms-accent-strong)]" /> All systems operational</div></div>
      </aside>
