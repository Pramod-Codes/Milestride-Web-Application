...
function FleetActivity({ hub, vehicles: hubVehicles, theme }: { hub: Hub | null; vehicles: typeof vehicles; theme: "dark" | "light" }) {
  ...
  return <div className={`absolute bottom-5 left-5 z-20 w-[min(370px,calc(100%-2.5rem))] rounded-xl border border-[var(--ms-border)] p-4 shadow-2xl backdrop-blur-md ${theme === "dark" ? "bg-[#171b19]/95" : "bg-white/95"}`}><div className="mb-4 flex items-start justify-between"><div><p className="text-xs font-semibold">Fleet activity</p><p className="mt-1 text-[10px] text-[var(--ms-muted)]">{hub.name} · {hub.available} available vehicles</p></div><span className={`rounded-full px-2 py-1 text-[9px] font-semibold ${hub.state === "critical" ? "status-red" : hub.state === "warning" ? "status-amber" : "status-green"}`}>{hub.state}</span></div><div className="mb-4 grid grid-cols-3 divide-x divide-[var(--ms-border)]"><MapMetric value={String(hub.available)} label="Available" color="text-[var(--ms-accent-strong)]" /><MapMetric value={String(moving)} label="Moving now" color="text-[#69a7de]" /><MapMetric value={String(attention)} label="Attention" color="text-[#e6b542]" /></div><div className="mb-2 flex items-center justify-between"><p className="text-[10px] font-semibold text-[var(--ms-muted)]">Available fleet ({hub.available})</p><span className="text-[9px] text-[var(--ms-muted)]">Battery status</span></div><div className="max-h-36 space-y-1.5 overflow-y-auto">{fleetRows.map((vehicle) => <div key={vehicle.id} className="flex items-center justify-between rounded-md border border-[var(--ms-border)] px-2.5 py-2 text-[10px]"><span className="font-medium">{vehicle.id}</span><span className="text-[var(--ms-muted)]">{vehicle.status} · {vehicle.battery}%</span></div>)}</div></div>;
}
